...
"""
    ...
    async def _get_completion(self, prompt: str, temperature=0.2) -> str:
        client = await self.synthesis_model.get_client(endpoint=ENDPOINT__CHAT_COMPLETIONS)
    ...
    async def _get_rank(self, prompt: str, inputs: List[str]) -> List[int]:
        client = await self.reranker_model.get_client(endpoint=ENDPOINT__CHAT_COMPLETIONS)
